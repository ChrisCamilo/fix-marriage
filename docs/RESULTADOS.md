...
| | |
|---|---|
| `patches.txt` | 2.678 linhas desde 2026-09-24 (+65 de cauda de IDR, `dados/patches_cauda.txt`; `mapa` idêntico antes e depois). Antes: 2.613 (+781: cabeçalhos de slice de 569 quadros, `dados/patches_cabecalho.txt`). Entraram 791; 12, de 6 quadros, estavam erradas e saíram no mesmo dia (armadilha 57); depois entraram 2 (1597 e 1610), achadas quando o 1595 deixou de contar como IDR |
...
| `verify` com `BASE_N=1338` | **0 válidos, 12 falsos** explicados, 1.328 pulados (desde 2026-09-24, com os 65 de cauda) — com o critério de ocultação nenhum reparo fecha um quadro sozinho (ver `AGENTS.md`) |
...
uma interpolação que parecia perfeita, ignorando 30 KB de dado. Saiu a linha
`77528965 2`, entrou `77528966 4`; o `77528961 0` é o `frame_num` e fica. Agora o
dado real decodifica até o MB 989 e o resto é ocultação de vizinhos bons. O
`serie` com `TARJA=1` ainda o aceita (tarja 16); os outros 166 bons seguem byte a
byte iguais. Virou alvo de reparo: dano no corpo a partir da fileira 8, numa ilha
com as referências intactas.
...
o dano continua no corpo. O ganho é que toda busca futura nesses 569 quadros
parte de um cabeçalho correto; 179 quadros que não geravam imagem passam a gerar.
